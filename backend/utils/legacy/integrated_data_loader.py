...
class IntegratedDataLoader:
    ...
    async def _load_trialtrove_data(self):
        ...
        try:
            ...
            df = read_regulatory_csv("combined_trial_trove.csv", low_memory=False)
    ...
    async def _load_sitetrove_data(self):
        ...
        try:
            ...
            df = read_regulatory_csv("combined_site_trove.csv", low_memory=False)
            df = clean_dataframe_for_json(df)
    ...
    async def _load_claims_data(self):
        ...
        try:
            ...
            df = read_regulatory_csv("claims/combined_claims.csv", low_memory=False)
            df = clean_dataframe_for_json(df)
    ...
    async def _load_fda_labels(self):
        ...
        try:
            ...
            df = read_regulatory_excel("FDA_Structured_Labels.xlsx")
            df = clean_dataframe_for_json(df)
    ...
    async def search_trials(self, filters: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for key, value in filters.items():
            if key == "search_term" and value:
                ...
                df = df[
                    df.astype(str)
                    .apply(lambda x: x.str.lower().str.contains(search_term, na=False))
                    .any(axis=1)
                ]
